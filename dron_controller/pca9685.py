...
class PCA9685:
    ...
    def set_channel_pwm(self, channel: int, pwm_amount: int):
        """
        ...
        phase_begin, phase_end = self.get_phase_cycle(channel, pwm_amount)

        # Start writing to the specified channel
        self.write_channel_pwm(channel, phase_begin, phase_end)

    def get_phase_cycle(self, channel: int, pwm_amount: int) -> tuple:
        """Calculate the on and off phases for a given PWM amount.

        Args:
            channel (int): The channel number (0 to 15).
            pwm_amount (int): PWM amount as a percentage (0 to 100).
        ...
        pwm_amount = max(0, min(pwm_amount, 100))
        ...
        phase_begin = 0
        phase_end = int((pwm_amount / 100.0) * total_range)
    ...
    PWM_MAX_VALUE = 4095  # 12-bit resolution maximum value
    ...
    def set_ESC_PWM(self, channel: int, pulse_width_us: int):
        ...
        """
        if pulse_width_us < 1000 or pulse_width_us > 2000:
            raise ValueError("Pulse width must be between 1000 and 2000 microseconds.")

        # Calculate the phase values using the scaling factor
        pulse_width_ticks = int(pulse_width_us * self.PWM_SCALE_FACTOR)

        # Set the PWM for the specified channel
        self.write_channel_pwm(channel,0, pulse_width_ticks)  # Set phase_end to 0 for ESC
...
if __name__ == "__main__":
    try:
        ...
        while pulse_width_us <= 2000:
            ...
            pca9685.set_ESC_PWM(0, pulse_width_us)

            # Print the current pulse width for debugging purposes
            print(f"Channel {0}: ESC pulse width set to {pulse_width_us} μs")
